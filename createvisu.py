...
# ================== STYLE ==================
FIG_PX_W, DPI = 1030, 100   # largeur fixe ; hauteur calculée dynamiquement
FIG_W = FIG_PX_W / DPI
...
def compute_rows(
    ...
    """
    Retourne : (Secteur, Perf 5D, Perf 1M, Perf YTD)
    -> on NE calcule plus 3M.
    """
...
def _draw_gauge(ax, x, y, w, h, value, cap):
    """
    Jauge + texte unique en noir, centré dans la case.
    """
    ax.add_patch(
        Rectangle(
            (x, y),
    ...
    if value is not None and not np.isnan(value) and cap > 0:
        ...
        bar_color = COLOR_BAR_POS if value >= 0 else COLOR_BAR_NEG
...
def draw_panel_legacy(
    ...
    # --- largeurs de colonnes : Secteur + 3 colonnes de perf (5D, 1M, YTD) ---
    c0 = int(0.46 * w)      # Secteur
    c1 = int(0.18 * w)      # 5D
    c2 = int(0.18 * w)      # 1M
    c3 = w - (c0 + c1 + c2) # YTD
    col_w = [c0, c1, c2, c3]

    headers = ["Secteur", "Perf 5D", "1M", "Perf YTD"]
    ...
    for i, (name, p5d, p1m, pytd) in enumerate(rows):
        ...
        _draw_gauge(ax, gx + 6, ry + 4, col_w[3] - 12, bar_h, pytd, CAP_YTD)
...
def build_legacy_two_panels(
    ...
    # hauteur dynamique pour ne pas couper Chimie / Auto / Media
    ...
    ax = plt.axes([0, 0, 1, 1])
    ax.axis("off")
    ax.set_xlim(0, FIG_PX_W)
    ax.set_ylim(0, fig_px_h)
    ax.invert_yaxis()
